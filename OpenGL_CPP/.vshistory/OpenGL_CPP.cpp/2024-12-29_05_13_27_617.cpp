...
int main()
{
	...
	for (unsigned int i = 0; i < 6; i++)
	{
		...
		unsigned char const* data = stbi_load(facesCubeMap[i].c_str(), &width, &height, &nrChannels,NULL);
		if (data)
		{
	...
	uint32_t programID = LoadShaders("\common\\SimpleVertexShader.vertexshader", "\common\\SimpleFragmentShader.fragmentshader");
