...
int main()
{
	...
	*/
	...
	do {
		...
		glDrawElements(GL_TRIANGLES, 36, GL_UNSIGNED_INT, &indexBufferSquareOne);

		glUniformMatrix4fv(MatrixIDS, 1, GL_FALSE, value_ptr(MVPTwo));
		glDrawElements(GL_TRIANGLES, 3, GL_UNSIGNED_INT, &indexBufferTriangle);

		glUniformMatrix4fv(MatrixIDS, 1, GL_FALSE, value_ptr(MVPThree));
		glDrawElements(GL_TRIANGLES, 36, GL_UNSIGNED_INT, &indexBufferSquareTwo);
