...
int main()
{
	...
	do {
		...
		auto MVPOne = cam.GetProjectionMatrix() * cam.GetViewMatrix() * ModelSquareOne; // Combine them into the MVP matrix

		auto ModelSquareTwo = glm::mat4(1.0f);
		ModelSquareTwo = translate(ModelSquareTwo, translateSquareTwo);
		auto MVPThree = cam.GetProjectionMatrix() * cam.GetViewMatrix() * ModelSquareTwo; // Combine them into the MVP matrix
		...
		auto MVPTwo = cam.GetProjectionMatrix() * cam.GetViewMatrix() * ModelTriangle; // Combine them into the MVP matrix
