...
int main()
{
	...
	do {
		...
		view = glm::mat4(glm::mat4(glm::lookAt(cam.m_cameraPos.get(), cam.m_cameraPos.get() + cam.m_cameraFront.get(), cam.m_cameraUp.get())));
		projection = glm::perspective(glm::radians(45.0f), static_cast<float>(width) / height, 0.1f, 100.0f);
		glUniformMatrix4fv(glGetUniformLocation(skyboxID, "view"), 1, GL_FALSE, value_ptr(view));
		glUniformMatrix4fv(glGetUniformLocation(skyboxID, "projection"), 1, GL_FALSE, value_ptr(projection));
