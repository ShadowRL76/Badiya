...
int main()
{
	...
	do {
		...
		view = glm::mat4(glm::mat4(glm::lookAt(cam.m_cameraPos, cam.m_cameraPos + cam.m_cameraFront, cam.m_cameraUp)));
		projection = glm::perspective(glm::radians(45.0f), static_cast<float>(width) / height, 0.1f, 100.0f);
		glUniformMatrix4fv(glGetUniformLocation(skyboxID, "view"), 1, GL_FALSE, glm::value_ptr(view));
