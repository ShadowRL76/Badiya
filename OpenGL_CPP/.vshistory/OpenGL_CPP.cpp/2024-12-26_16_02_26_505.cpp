...
int main()
{
    ...
    ImGuiManager::Params pm{ (camera(window.get())), window.get(), translateSquareOne, translateSquareTwo, translateTriangle }; 
    do {
        ...
        camera(window.get()).Controls(window.get()); 
        camera(window.get()).GetProjectionMatrix();
        camera(window.get()).GetViewMatrix();
        ...
        glm::mat4 MVPOne = camera(window.get()).GetProjectionMatrix() * camera(window.get()).GetViewMatrix() * ModelSquareOne; // Combine them into the MVP matrix      


        auto ModelSquareTwo = glm::mat4(1.0f); 
        ModelSquareTwo = glm::translate(ModelSquareTwo, translateSquareTwo); 
        glm::mat4 MVPThree = camera(window.get()).GetProjectionMatrix() * camera(window.get()).GetViewMatrix() * ModelSquareTwo; // Combine them into the MVP matrix    

        // Get the location of the MVP uniform
        auto ModelTriangle = glm::mat4(1.0f);
        constexpr float angle(glm::radians(90.0));  
        ModelTriangle = glm::rotate(ModelTriangle, angle, glm::vec3(0, 1, 0)); 
        ModelTriangle = glm::translate(ModelTriangle, translateTriangle);
        glm::mat4 MVPTwo = camera(window.get()).GetProjectionMatrix() * camera(window.get()).GetViewMatrix() * ModelTriangle; // Combine them into the MVP matrix     
